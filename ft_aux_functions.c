...
/*   Updated: 2026/07/10 01:25:11 by guilherme        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ft_handle_base(char c, va_list args)
{
	...
	if (c == 'u')
	{
		...
		return (ft_numlen_base(unb, "0123456789"));
	}
	else if (c == 'x')
	{
		ft_putnbr_base_fd(unb, "0123456789abcdef", 1);
		return (ft_numlen_base(unb, "0123456789abcdef"));
	}
	else if (c == 'X')
	{
		ft_putnbr_base_fd(unb, "0123456789ABCDEF", 1);
...
static int	ft_handle_int(char c, va_list args)
{
	...
	else if (c == 'd' || c == 'i')
	{
		...
		ft_putnbr_fd(nbr, 1);
		if (nbr < 0)
			return (ft_numlen_base((unsigned long)-nbr, "0123456789") + 1);
		return (ft_numlen_base((unsigned int)nbr, "0123456789"));
	}
	else if (c == 's')
	{
...
int	get_type(char c, va_list args)
{
	...
	else if (c == 'p')
	{
		...
		return (2 + ft_numlen_base((unsigned long)vd, "0123456789abcdef"));
	}
	else if (c == '%')
	{
		ft_putchar_fd('%', 1);
